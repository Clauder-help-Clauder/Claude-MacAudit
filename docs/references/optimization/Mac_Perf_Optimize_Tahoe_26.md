# Mac 效能优化 — macOS Tahoe 26.4 版

> 适用系统：macOS Tahoe 26.4（Apple Silicon M4 Max，64GB 统一内存，2TB SSD）
...
- macOS Tahoe 26 引入了 Apple Intelligence 服务（`intelligenceflowd`、`intelligenceplatformd` 等），需要额外禁用
- `launchctl bootout` / `launchctl disable` 是正确语法（`launchctl unload -w` 已弃用）
- SIP 保持启用，所有操作不需要关闭 SIP
- Stage Manager 默认开启，需要手动关闭
- Liquid Glass UI 特效更重，关闭透明度收益更明显
...
defaults write com.apple.dock expose-animation-duration -float 0.1  # Mission Control（设 0.1 避免 Tahoe 显示异常）
...
# 系统级减少动态效果 + 透明度（Tahoe Liquid Glass 特效很重）
...
```bash
#!/bin/bash
# === macOS Tahoe 26.4 — 关闭非必要服务 ===
# 语法：launchctl bootout + disable（Tahoe 正确方式）
...
echo "=== 1. Siri / Apple Intelligence（Tahoe 新增）==="
SIRI_AI=(
...
| `com.apple.modelcatalogd` | Tahoe 新增，ML 模型目录服务，Ollama 可能依赖 |
| `com.apple.modelmanagerd` | ML 模型管理器，本地 AI 推理依赖 |

> **注意**：macOS Tahoe 的 `modelcatalogd` 和 `modelmanagerd` 是 Apple 本地 AI 基础设施，如果你用 Apple 的 Core ML 或 Foundation Models 框架，不要关闭。Ollama 使用自己的运行时不受影响，但以防万一保留。
...
### Step K：关闭 Liquid Glass 动态模糊（Tahoe 专用）

Tahoe 的 Liquid Glass UI 引入了实时窗口背景模糊效果，持续消耗 GPU。`reduceTransparency` 只关闭了透明度，模糊是单独的开关：

```bash
defaults write com.apple.universalaccess reduceBlurring -bool true
```

### Step L：Ollama 升级到 0.19+（MLX 加速，Tahoe + Apple Silicon 专用）
...
> 来源：[MacRumors 2026-03-31](https://www.macrumors.com/2026/03/31/ollama-now-runs-faster-apple-silicon-macs/)

### Step M：Ollama 内存管理环境变量
...
### Step O：Quick Note 热角修饰键清零（Tahoe 专用）

macOS Tahoe 默认右下角触发 Quick Note，即使热角设为 0 也可能被修饰键组合触发：
...
### 注意：暂不更新到 macOS Tahoe 26.5 Beta

26.5 Beta 1（2026-03-30 发布）包含 WindowServer 大幅重写和 SMB 客户端重写，可能引入不稳定因素。建议等正式版。
...
### Step Q：M4 Max 本地大模型热管理与性能优化

M4 Max 拥有 64GB 统一内存和 40 核 GPU，是本地跑大模型的顶级配置。双风扇主动散热系统，热管理压力远小于 M3/M4 基础版。
...
| Q4_K_M | ✅ 首选 | ~4GB | ~40GB |
| Q5_K_M | ✅ 可用 | ~5GB | ~48GB |
| Q6_K | ⚠️ 70B 可能紧张 | ~6GB | ~55GB |
| Q8_0 | ⚠️ 仅 7B-13B | ~7GB | 超 64GB |
...
> M4 Max 温度墙约 105°C，正常负载下不应超过 85°C。如果持续超过 90°C 说明散热有问题（检查风扇通道是否被堵）。

### Step Q-2：Metal GPU 全量卸载验证
...
M4 Max 有 546 GB/s 内存带宽，Ollama 推理性能**直接受内存带宽限制**（不是 CPU/GPU 计算瓶颈）。

**关键规则**：
- `OLLAMA_MAX_LOADED_MODELS=2` 是 64GB 的实用上限，超过 2 个模型同时驻留会严重争抢带宽
- Ollama 推理 + Xcode 编译**不要同时跑** — Xcode linker 阶段吃大量内存带宽，会导致 Ollama 推理速度骤降
...
### Step Q-4：Tahoe 26.2-26.3 WindowServer 卡顿修复

macOS Tahoe 26.2-26.3 有已知的 WindowServer 卡顿 bug（26.4 已部分修复）。如果仍有偶发卡顿：

```bash
# 重置 WindowServer 缓存
sudo rm -rf /private/var/folders/*/*/com.apple.WindowServer*

# 重建 Launch Services 数据库（修复应用图标/打开方式异常）
/System/Library/Frameworks/CoreServices.framework/Frameworks/LaunchServices.framework/Support/lsregister -kill -r -domain local -domain system -domain user
```

> 执行后需要重启。仅在遇到卡顿时使用，不需要常规执行。
...
> 右上角通知中心完全消失，所有通知（含系统通知）不再弹出。还原：`launchctl enable gui/501/com.apple.notificationcenterui`

### Step S：caffeinate 升级为系统级 LaunchDaemon

当前 caffeinate 在 `~/Library/LaunchAgents/`（用户级），需要用户登录才生效。升级为系统级，开机即生效：
...
echo ""
echo "=== Liquid Glass 模糊 ==="
echo -n "  reduceBlurring: "; defaults read com.apple.universalaccess reduceBlurring 2>/dev/null || echo "未设置"
...
# 还原热角（1=Mission Control, 其他值见 defaults 文档）
