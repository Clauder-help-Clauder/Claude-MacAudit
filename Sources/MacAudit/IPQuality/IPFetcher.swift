...
//  MacAudit
//
//  本地 IP 信息获取模块（Phase A）。
//  纯 shell 命令实现，无需 API Key。支持公网 IPv4/IPv6 多源并行竞争获取、
//  反向 DNS 查询、whois 信息查询、系统 DNS/代理/网关/网络接口采集。
//
...
struct IPFetcher: Sendable {
    ...
    /// - Returns: 公网 IPv4 地址，所有源均失败时返回 nil
    ...
    /// - Parameter executor: Shell 命令执行器
    /// - Returns: 公网 IPv6 地址，所有源均失败时返回 nil
    ...
    /// - Returns: 反向 DNS 解析结果（如 "dns.google."），失败返回 nil
    ...
    ///   - ip: 待查询的 IPv4 地址
    ///   - executor: Shell 命令执行器
    /// - Returns: 元组 (org: 组织名, country: 国家代码)，失败时对应字段为 nil
    ...
    /// 获取系统 DNS 服务器
    /// - Parameter executor: Shell 命令执行器
    /// - Returns: 逗号分隔的 DNS 服务器列表，失败返回 "N/A"
    ...
    /// 获取本地网络接口 IP
    /// - Parameter executor: Shell 命令执行器
    /// - Returns: 逗号分隔的本地 IP 列表（排除 127.0.0.1），失败返回 "N/A"
    ...
    /// 从 whois 输出行中提取冒号后的值
    /// - Parameter line: whois 输出的单行文本（如 "orgname: Google LLC"）
    /// - Returns: 冒号后去除空白的值，无冒号或值为空时返回 nil
