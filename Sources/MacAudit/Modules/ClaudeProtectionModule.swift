...
//  检测危险环境变量（风控风险）、安全基线配置、网络防护、防火墙状态等，
//  并提供 deferredChecks 展示低影响信息项。
//
...
/// v2 重构依据：Claude Code 封号机制逆向分析文档
/// 核心原则：融入而非消失。关闭遥测本身是风险行为。
struct ClaudeProtectionModule: AuditModule {
    /// 显式初始化器
    init() {}
    ...
    let name = "AI服务调优"
    /// 模块功能描述
    let description = "AI 服务效率与安全态势检测"

    /// A 组：安全环境变量（正向检测，期望=设置）
    ...
    /// Apple 系统遥测防护的添加/取消命令说明
    private let telemetryAddCmd = """
...
"""
    ...
    /// 生成所有检测项：风险信号检测（B组）、危险环境变量、安全基线（A组）、网络防护、防火墙和安全工具、代理辅助检测
...
"""
        ...
        // A组独立检测（每项附带 enableNote/disableNote 作为 description 和 fixCommand）
...
""",
    ...
    /// 延迟检测项（低影响信息展示），包括隐藏升级命令、设备指纹、git 邮箱泄露、时区/语言信号、Apple 遥测等
