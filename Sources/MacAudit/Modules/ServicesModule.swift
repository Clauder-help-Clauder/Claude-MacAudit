...
//
//  M6: 服务状态模块
//  检测 launchd 服务的禁用状态，按分组管理可禁用的系统服务，
//  包括 Siri/AI、媒体/娱乐、照片/地图/社交、iCloud/家庭、遥测/分析、共享/Handoff 等类别。
//  通过 launchctl print-disabled 一次性读取所有禁用状态，按服务逐项判定。
//

import Foundation

/// M6: 服务状态模块 — 检测 launchd 服务禁用状态
struct ServicesModule: AuditModule {
    /// 显式初始化器
    init() {}
    ...
    let name = "服务状态"
    /// 模块功能描述
    let description = "launchd 服务禁用状态检测（期望禁用不需要的服务）"
    ...
    /// 可禁用服务定义列表，按分组：Siri/AI、媒体/娱乐、照片/地图/社交、iCloud/家庭、遥测/分析、共享/Handoff
    ...
    /// 从 ServiceDef 构建 AuditCheck，自动生成包含 hint 和分组信息的 description
...
【批量禁用同组服务】在「服务管理」菜单（选项7）中按分组批量操作
"""
        return AuditCheck(
            id: "m6.\(svc.name)",
    ...
    /// 执行服务状态检测：一次性读取 launchctl print-disabled，逐项判定禁用/启用/未管理状态
