...
//
//  M9: 终端环境模块
//  检测 Shell 配置和环境变量，包括默认 Shell、代理设置、
//  all_proxy_on/off 函数、Homebrew analytics、Git 配置、SSH 配置、
//  ulimit 限制、危险别名检测、语言环境信号、zsh_history 中文命令、dotfiles 数量等。
//
...
/// M9: 终端环境模块
struct ShellModule: AuditModule {
    /// 模块唯一标识
    let id = "shell"
    /// 模块显示名称
    let name = "终端环境"
    /// 模块功能描述
    let description = "Shell 配置和环境变量检测"

    /// 生成终端环境检查项：默认 Shell、代理配置、Git/SSH 配置、ulimit、语言信号、dotfiles 等
