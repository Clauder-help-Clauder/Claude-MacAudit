//
//  DevEnvironmentModule.swift
//  MacAudit
//
//  M11: 开发工具模块
//  检测开发工具链（Xcode CLT、Homebrew、Node.js、Python、Rust、Go、Java 等）
//  及效率工具（ripgrep、fzf、jq 等）的安装和配置状态，
//  并检测 Ollama/MLX 等 AI 工具和文件描述符限制等系统参数。
//
...
/// M11: 开发工具模块
struct DevEnvironmentModule: AuditModule {
    /// 模块唯一标识
    let id = "dev"
    /// 模块显示名称
    let name = "开发工具"
    /// 模块功能描述
    let description = "开发工具链和环境配置检测"
    ...
    /// 所有开发工具定义，按类别分组：基础工具链、运行时、Sequoia/Tahoe 专属、Rust 组件、Git 工具链、效率工具、容器/AI、AI CLI、Xcode 清理、brew 统计、文件描述符限制、补充工具
...
""",
    ...
    /// 根据 macOS 版本和 CPU 架构筛选适用的工具检查项，转换为 AuditCheck 数组
