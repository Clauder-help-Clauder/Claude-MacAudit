...
/// M13: IP 质量检测模块
public struct IPQualityModule: AuditModule {
    public init() {}

    private var cache: IPCache { IPCache.shared }

    public let id = "ip_quality"
    public let name = "IP 质量检测"
    public let description = "IP 地址质量和风险评估"
    ...
    public func run(
        ...
        let cache = IPCache.shared

        // === 尝试使用缓存的 IPv4 ===
        let cachedIP = cache.getIPv4()

        if let cachedIP {
            // 有缓存 IP，但仍需验证网络可用性
            InteractiveUI.updateProgress(module: name, current: 0, total: total)
            let netCheck = await executor.run("curl -s --max-time 3 https://ifconfig.me 2>/dev/null")
            ...
            // Phase A: 使用缓存 IP，只获取本地信息
            let phaseAResults = await runPhaseAWithCachedIP(cachedIP, executor: executor)
            results.append(contentsOf: phaseAResults)

            // Phase B/C/D 并行（不依赖 Phase B 完成）
            async let phaseBResults = GeoIPService.fetch(ip: cachedIP)
            async let phaseCResults = DNSBLChecker.check(ip: cachedIP, executor: executor)
            async let phaseDResults = MailChecker.check(executor: executor)

            let bResults = await phaseBResults
            let cResults = await phaseCResults
            let dResults = await phaseDResults
            results.append(contentsOf: bResults)
            results.append(contentsOf: cResults)
            results.append(contentsOf: dResults)

            cache.store(geoIPResults: bResults)
        } else {
            // === 无缓存，走完整流程 ===
            InteractiveUI.updateProgress(module: name, current: 0, total: total)
            ...
            if !publicIP.isEmpty {
                cache.store(ipv4: publicIP)

                // === Phase B/C/D 并行（不再等 Phase B 完成） ===
                async let phaseBResults = GeoIPService.fetch(ip: publicIP)
    ...
    // MARK: - Phase A: Local checks

    private func runPhaseAWithCachedIP(_ cachedIP: String, executor: ShellExecutor) async -> [AuditResult] {
        let checks = phaseAChecks()
        var results: [AuditResult] = []

        results.append(.info(check: checks[0], actual: cachedIP))

        let v6 = cache.getIPv6()
        results.append(.info(check: checks[1], actual: v6 ?? "不可用"))
        ...
        results.append(.info(check: checks[2], actual: ifs.isEmpty ? "N/A" : ifs))
        results.append(.info(check: checks[3], actual: dnsResult.isEmpty ? "N/A" : dnsResult))
        results.append(.info(check: checks[4], actual: proxyResult))
        results.append(.info(check: checks[5], actual: gw.isEmpty ? "N/A" : gw))

        async let rdns = IPFetcher.reverseDNS(ip: cachedIP, executor: executor)
        async let whois = IPFetcher.whoisInfo(ip: cachedIP, executor: executor)

        let rdnsResult = await rdns
        let whoisResult = await whois

        results.append(.info(check: checks[6], actual: rdnsResult ?? "无记录"))
        results.append(.info(check: checks[7], actual: whoisResult.org ?? "N/A"))
        results.append(.info(check: checks[8], actual: whoisResult.country ?? "N/A"))
    ...
    private func runPhaseA(executor: ShellExecutor) async -> [AuditResult] {
        ...
        // IPv4
        if let v4 {
            results.append(.info(check: checks[0], actual: v4))
        } else {
            results.append(.error(check: checks[0], error: "无法获取公网 IPv4"))
        }
        ...
        if let v4 {
            ...
            results.append(.skip(check: checks[6], reason: "无公网 IP"))
            results.append(.skip(check: checks[7], reason: "无公网 IP"))
            results.append(.skip(check: checks[8], reason: "无公网 IP"))
        }
    ...
    private func runPhaseAOffline(executor: ShellExecutor) async -> [AuditResult] {
        ...
        results.append(.error(check: checks[0], error: "网络不可用"))
        results.append(.error(check: checks[1], error: "网络不可用"))

        let ifs = await IPFetcher.localInterfaces(executor: executor)
        results.append(.info(check: checks[2], actual: ifs.isEmpty ? "N/A" : ifs))

        let dns = await IPFetcher.dnsServers(executor: executor)
        results.append(.info(check: checks[3], actual: dns.isEmpty ? "N/A" : dns))

        let proxy = await IPFetcher.proxyConfig(executor: executor)
        results.append(.info(check: checks[4], actual: proxy))
        ...
        // 无网络，跳过 rdns/whois
        for i in 6...8 {
            results.append(.skip(check: checks[i], reason: "网络不可用"))
        }
